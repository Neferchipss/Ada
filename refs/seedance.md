...
> - **Higgsfield element syntax is markdown-link form: `@[Name](uuid)`** — e.g. `@[Feather](a700a2a7-e4f3-4a44-b334-ad9a75846f84)`. Name is case-sensitive; the UUID must be present. Repeat the full `@[Name](uuid)` at EVERY mention, not just the first. The skill's `<<<image_n>>>` legend convention is Seedance-native and does NOT apply to Higgsfield.
> - **When a generation is cinematically RIGHT but has defects, patch the defects surgically — never rewrite.** Live failure (2026-07-31): a prompt produced great camera work plus three artifacts; I rebuilt it from scratch around safety constraints and killed the cinema. He noticed instantly. Isolate the exact clause causing each artifact, replace only that clause, keep every other word verbatim. Same shape as the general rule that the opposite of a wrong choice is not automatically right.
> - **Momentum must survive a continuation join.** Matching the last FRAME is not enough — match the VECTOR. If the reference ends with the subject rising and the camera tilting up, the next generation must keep rising and keep tilting up for a beat, then turn over on its own arc (apex → hang → sink). Reversing direction on the first frame after a seamless join reads as rewind, even though the still matches perfectly. Extract the reference's last frame AND describe what was moving which way when it ended.
> - **A slow subject needs camera-side drama, not more speed.** When a beat is "just drifting, no drama", the fix is never to speed the subject up (that breaks the natural-motion read). Add parallax instead: orbit the subject so the background swings, arc past a foreground occluder, rack focus between subject and the people behind, drop to a low angle, or give the subject one brief physical interaction with the set (settling on a surface and lifting off again). Drama is the camera's job and the staging's job.
> - **Separate the CAMERA's energy from the SUBJECT's rate.** A crane descent, hard cuts and a tilt-up can stay fully cinematic while the subject moves slowly. Speed leaks into the subject through camera-side words — *accelerates, tears past, chasing, rushing, sweeping, without slowing* — so scrub those and state the subject's rate positively instead: "unhurried, buoyed on still air, descending more than it rises, gravity always slightly winning." A feather that glides naturally sinks while it drifts; one that holds altitude reads as powered flight.
> - Known artifact→clause map for a drifting flat asset: *through-plane flip verb* ("rolls onto its face") → doubled/two-sided asset · *asset described as small/distant in frame* → resolves as a bird, sprouts wings · *asset's SHADOW described* → shadow renders two-sided · *flight verbs* (climbs, banks, glides, soars) → creature behaviour · *many cuts in a short duration* → speed-ramped "fast forward" feel.
...
> - **Never ask a flat, one-sided asset (a feather, a leaf, a card, a banner) to roll/flip/turn edge-on.** A through-plane rotation makes the model render a reverse side it has no reference for — the observed failure is a "double-sided feather": a second eye, a doubled shaft, or the asset splitting. Keep the hero face toward camera for the whole shot and put the motion IN-PLANE (rotation within the picture plane, drift, tilt, scale). It still reads as tumbling. State it explicitly: "one shaft, one eye, eye-side toward camera throughout; never flips, never turns edge-on."
> - **Continuation workflow (multi-generation sequences).** When a sequence is longer than one generation, each part must END MID-MOTION — never let the subject exit frame or resolve (an exit reads as an ending and gives the next generation nothing to inherit). Then the next prompt opens with an explicit continuation clause naming the reference video and re-anchoring four things: subject size in frame, screen direction of travel, light/time-of-day, and camera height/mode. Without all four, the model resets the geometry at the seam.

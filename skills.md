...
- **motion-dev-animations skill** — host-conditional, not portable: on hosts where it's installed (`199-biotechnologies/motion-dev-animations-skill`, user + relevant project scope), pairs with ui-ux-pro-max for site work — reach for it on any real animation ask (spring physics, gestures, scroll reveals, layout/page transitions) in React/Next/Svelte/Astro instead of hand-writing Motion (formerly Framer Motion) calls from memory. Not the same thing as Framer.com's own website-builder product/agent — different company relationship at this point, shared name only; don't conflate the two. Same check-before-assuming rule as its pair.
...
- **Adversarial pre-write review, for consequential/architecture-level decisions** — before committing a big design to a file (something that'll govern real behavior, not a one-off task), spawn an independent read-only review grounded against REAL current files and past incidents, not just a paraphrase of the conversation. Proven on the adasplit protocol revamp (2026-07-08): handing a subagent a compressed "verbatim" summary instead of the real file got caught immediately once it went and read the actual file itself; grounding the critique in a concurrent live session's real logs (a genuine conflicting-review incident, a stale-read miss, an evidence-overwrite dispute) surfaced gaps a pure design conversation never would have — race conditions, semantic conflations, missing edge cases — all fixed before the file existed rather than after. Worth the cost specifically when the thing being written is consequential enough that a second redesign pass would be expensive; not for routine tasks.

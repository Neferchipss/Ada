...
- **Animation reviews MUST declare loop vs. one-shot in the brief.** A **looping** cycle (walk/run/gallop/idle) is judged on seam continuity too — the start and end frames must join without a hitch, velocities matched across the wrap — so agy explicitly checks the loop point. A **one-shot** action (a strike, a jump, a stagger) has no seam expectation and is judged on anticipation/arc/settle. The two demand different critiques; omitting which one it is invalidates the verdict (agy may pass a clip that reads fine start-to-end but jerks at the loop wrap, or flag a "missing settle" on something that's meant to cycle). State it every time, alongside the intended feel.
...
- **A visual verdict MUST land as its own `from: "agy"`, `type: "review"` channel entry — never folded into the calling agent's `status`/`done` line.** This is what keeps the review independently visible instead of hearsay. Enforcement: if a particular consult path returns agy's verdict only as text to the caller (a headless one-shot invocation without folder access in that call), the **caller must post it as a distinct agy-attributed `review` line** (`from: "agy"`, the verdict, region/time_range, and a note that it was relayed) the same turn — a verdict buried inside "asked Gemini, it said APPROVE" is a protocol miss, not a logged review. Real incident this happened: a Gemini APPROVE existed only inside a Codex `status` line with no agy-authored entry, so the review couldn't be told apart from the author's own claim.
